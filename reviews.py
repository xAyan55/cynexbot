...
logger = logging.getLogger("KINETICHOST.Reviews")
...
class ReviewSubmitModal(discord.ui.Modal, title="Submit a Service Review"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        try:
            ...
            err = KINETICHOSTErrorContainer("Invalid Rating", "Please enter an integer between 1 and 5 for your rating.")
        ...
        if screenshot_url and not (screenshot_url.startswith("http://") or screenshot_url.startswith("https://")):
            err = KINETICHOSTErrorContainer("Invalid Screenshot URL", "Please enter a valid HTTP/HTTPS image URL.")
            await interaction.followup.send(view=err.build(), ephemeral=True)
            return
        ...
        if pub_channel:
            try:
                ...
                btn_helpful = Button(label="Helpful (0)", style=discord.ButtonStyle.secondary, custom_id=f"KINETICHOST:review:vote:helpful:{review_id}")
                btn_unhelpful = Button(label="Not Helpful (0)", style=discord.ButtonStyle.secondary, custom_id=f"KINETICHOST:review:vote:unhelpful:{review_id}")
                btn_report = Button(label="Report Review", style=discord.ButtonStyle.secondary, custom_id=f"KINETICHOST:review:report:{review_id}")
        ...
        success = KINETICHOSTSuccessContainer("Review Submitted", f"Thank you! Your review `{review_id}` has been published to {pub_channel.mention if pub_channel else 'the reviews channel'}.")
...
class ReviewDenyModal(discord.ui.Modal, title="Deny Review Confirmation"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        try:
            denied_layout = KINETICHOSTErrorContainer("Review Denied", f"Review `{self.review_id}` has been denied.")
        ...
        success = KINETICHOSTSuccessContainer("Review Denied Successfully", f"Review `{self.review_id}` status has been set to denied.")
        await interaction.followup.send(view=success.build(), ephemeral=True)
...
class Reviews(commands.Cog):
    ...
    async def on_interaction(self, interaction: discord.Interaction):
        ...
        if not custom_id or not custom_id.startswith("KINETICHOST:review:"):
            return
        ...
        if action == "approve":
            ...
            if not review:
                err = KINETICHOSTErrorContainer("Review Not Found", f"Metadata for `{review_id}` is missing.")
                await interaction.followup.send(view=err.build(), ephemeral=True)
                return
            ...
            # Edit Mod Message
            try:
                mod_ok = KINETICHOSTSuccessContainer("Review Approved", f"Review `{review_id}` has been approved.")
            ...
            success = KINETICHOSTSuccessContainer("Review Approved Successfully", f"Review `{review_id}` is published.")
            await interaction.followup.send(view=success.build(), ephemeral=True)
        ...
        # Vote helpful/unhelpful flow
            ...
            try:
                ...
                if action_row:
                    ...
                    if rev_row:
                        ...
                        btn_helpful = Button(label=f"Helpful ({help_count})", style=discord.ButtonStyle.secondary, custom_id=f"KINETICHOST:review:vote:helpful:{review_id}")
                        btn_unhelpful = Button(label=f"Not Helpful ({unhelp_count})", style=discord.ButtonStyle.secondary, custom_id=f"KINETICHOST:review:vote:unhelpful:{review_id}")
                        btn_report = Button(label="Report Review", style=discord.ButtonStyle.secondary, custom_id=f"KINETICHOST:review:report:{review_id}")
                        pub_layout.add_buttons(btn_helpful, btn_unhelpful, btn_report)
            ...
            success = KINETICHOSTSuccessContainer("Vote Logged", f"Your helpfulness vote has been successfully {msg_action}.")
            await interaction.followup.send(view=success.build(), ephemeral=True)
        ...
        # Report flow
            ...
            if settings and settings["mod_channel_id"]:
                ...
                if mod_channel:
                    try:
                        rep_card = KINETICHOSTWarningContainer("Review Reported Alert", f"Review `{review_id}` was reported by a customer.")
                        ...
                        del_btn = Button(label="Force Delete Review", style=discord.ButtonStyle.secondary, custom_id=f"KINETICHOST:review:forcedelete:{review_id}")
            ...
            success = KINETICHOSTSuccessContainer("Report Submitted", "Thank you for reporting. This review has been submitted to staff for inspection.")
            await interaction.followup.send(view=success.build(), ephemeral=True)
        ...
        # Force delete review (moderator action)
            ...
            if not is_admin:
                err = KINETICHOSTErrorContainer("Permission Denied", "Only administrators can force-delete reviews.")
                await interaction.followup.send(view=err.build(), ephemeral=True)
                return
            ...
            try:
                done = KINETICHOSTSuccessContainer("Review Force Deleted", f"Review `{review_id}` has been deleted from database by {interaction.user.mention}.")
                await interaction.message.edit(view=done.build())
            except Exception:
                pass
                
            success = KINETICHOSTSuccessContainer("Review Deleted", f"Review `{review_id}` deleted successfully.")
            await interaction.followup.send(view=success.build(), ephemeral=True)
    ...
    review_group = app_commands.Group(name="review", description="KINETICHOST reviews management system")
    ...
    async def submit_review(self, interaction: discord.Interaction):
        ...
        if not settings:
            err = KINETICHOSTErrorContainer("Configuration Error", "Reviews system is not set up on this server. Ask an administrator to run `/review setup`.")
    ...
    async def approve_review(self, interaction: discord.Interaction, review_id: str):
        ...
        if not row:
            err = KINETICHOSTErrorContainer("Review Not Found", f"Review `{review_id}` does not exist.")
            await interaction.followup.send(view=err.build(), ephemeral=True)
            return
            
        status, author_id, rating, service, msg_content, screenshot_url = row
        if status != "pending":
            warn = KINETICHOSTWarningContainer("Already Moderated", f"Review `{review_id}` is already `{status}`.")
        ...
        success = KINETICHOSTSuccessContainer("Review Approved", f"Review `{review_id}` approved and posted.")
        await interaction.followup.send(view=success.build(), ephemeral=True)
    ...
    async def deny_review(self, interaction: discord.Interaction, review_id: str, reason: str):
        ...
        success = KINETICHOSTSuccessContainer("Review Denied", f"Review `{review_id}` has been denied. Reason: **{reason}**")
        await interaction.followup.send(view=success.build(), ephemeral=True)
    ...
    async def delete_review(self, interaction: discord.Interaction, review_id: str):
        ...
        success = KINETICHOSTSuccessContainer("Review Deleted", f"Review `{review_id}` has been deleted from the database.")
        await interaction.followup.send(view=success.build(), ephemeral=True)
    ...
    async def list_reviews(self, interaction: discord.Interaction):
        ...
        if not rows:
            info = KINETICHOSTInfoContainer("No Reviews", "There are no approved reviews on this server yet.")
    ...
    async def reviews_stats(self, interaction: discord.Interaction):
        ...
        stats_card = KINETICHOSTInfoContainer("Reviews Analytics", f"**Server:** {interaction.guild.name}")
