...
logger = logging.getLogger("KINETICHOST.Suggestions")
    ...
    async def on_submit(self, interaction: discord.Interaction):
                ...
                if row:
                    try:
                        ...
                        if (now - dt).total_seconds() < 3600:
                            warn = KINETICHOSTWarningContainer("Duplicate Suggestion Detected", "You have already submitted a suggestion with this title in the last hour.")
        ...
        if suggest_channel:
            try:
                ...
                btn_up = Button(label="👍 Upvote (0)", style=discord.ButtonStyle.secondary, custom_id=f"KINETICHOST:suggest:vote:up:{suggestion_id}")
                btn_down = Button(label="👎 Downvote (0)", style=discord.ButtonStyle.secondary, custom_id=f"KINETICHOST:suggest:vote:down:{suggestion_id}")
                btn_stats = Button(label="📊 Stats", style=discord.ButtonStyle.secondary, custom_id=f"KINETICHOST:suggest:stats:{suggestion_id}")
        ...
        success = KINETICHOSTSuccessContainer("Suggestion Submitted", f"Your suggestion `{suggestion_id}` has been published to {suggest_channel.mention if suggest_channel else 'the suggestion channel'}.")
...
class Suggestions(commands.Cog):
    ...
    async def on_interaction(self, interaction: discord.Interaction):
        ...
        if not custom_id or not custom_id.startswith("KINETICHOST:suggest:"):
            return
        ...
        if action == "vote":
            ...
            async with aiosqlite.connect(DB_PATH) as db:
                ...
                if not status_row:
                    err = KINETICHOSTErrorContainer("Suggestion Missing", "The suggestion data could not be found.")
                    await interaction.followup.send(view=err.build(), ephemeral=True)
                    return
                    
                status, author_id = status_row
                if status in ("implemented", "denied"):
                    err = KINETICHOSTErrorContainer("Voting Closed", "This suggestion is already implemented or denied. Voting is disabled.")
                    await interaction.followup.send(view=err.build(), ephemeral=True)
                    return

                # Prevent self-voting
                if author_id == user_id:
                    err = KINETICHOSTErrorContainer("Self Voting Blocked", "You cannot vote on your own suggestions.")
                    await interaction.followup.send(view=err.build(), ephemeral=True)
                    return
            ...
            try:
                ...
                if s_data:
                    ...
                    btn_up = Button(label=f"👍 Upvote ({upvotes})", style=discord.ButtonStyle.secondary, custom_id=f"KINETICHOST:suggest:vote:up:{suggestion_id}")
                    btn_down = Button(label=f"👎 Downvote ({downvotes})", style=discord.ButtonStyle.secondary, custom_id=f"KINETICHOST:suggest:vote:down:{suggestion_id}")
                    btn_stats = Button(label="📊 Stats", style=discord.ButtonStyle.secondary, custom_id=f"KINETICHOST:suggest:stats:{suggestion_id}")
                    pub_layout.add_buttons(btn_up, btn_down, btn_stats)
            ...
            success = KINETICHOSTSuccessContainer("Vote Confirmed", f"Your vote has been successfully {msg_action}.")
            await interaction.followup.send(view=success.build(), ephemeral=True)
        ...
        # Stats view flow
            ...
            info = KINETICHOSTInfoContainer(f"Suggestion Statistics - {suggestion_id}", f"Real-time voting analytical tallies.")
    ...
    async def suggest(self, interaction: discord.Interaction, category: str, anonymous: bool = False):
        ...
        if not settings or not settings["suggest_channel_id"]:
            err = KINETICHOSTErrorContainer("Configuration Error", "Suggestions are not set up on this server yet. Ask an administrator to run `/suggestion setup`.")
    ...
    suggestion_group = app_commands.Group(name="suggestion", description="KINETICHOST suggestion management panel")
    ...
    async def setup_suggestions(self, interaction: discord.Interaction, channel: discord.TextChannel):
        ...
        success = KINETICHOSTSuccessContainer("Suggestions Setup Completed", f"Suggestions will be posted to {channel.mention}.")
        await interaction.followup.send(view=success.build(), ephemeral=True)
    ...
    async def approve_suggestion(self, interaction: discord.Interaction, suggestion_id: str, reason: str):
        ...
        if not row:
            err = KINETICHOSTErrorContainer("Suggestion Not Found", f"Suggestion `{suggestion_id}` is missing.")
            await interaction.followup.send(view=err.build(), ephemeral=True)
            return

        status, title, desc, category, anon, author_uid = row
        if status != "pending":
            warn = KINETICHOSTWarningContainer("Already Moderated", f"Suggestion `{suggestion_id}` status is already `{status}`.")
            await interaction.followup.send(view=warn.build(), ephemeral=True)
            return
        ...
        if settings and settings["suggest_channel_id"]:
            ...
            if s_channel:
                try:
                    ...
                    btn_up = Button(label=f"👍 Upvote ({up})", style=discord.ButtonStyle.secondary, custom_id=f"KINETICHOST:suggest:vote:up:{suggestion_id}")
                    btn_down = Button(label=f"👎 Downvote ({down})", style=discord.ButtonStyle.secondary, custom_id=f"KINETICHOST:suggest:vote:down:{suggestion_id}")
                    btn_stats = Button(label="📊 Stats", style=discord.ButtonStyle.secondary, custom_id=f"KINETICHOST:suggest:stats:{suggestion_id}")
                    pub_layout.add_buttons(btn_up, btn_down, btn_stats)
        ...
        success = KINETICHOSTSuccessContainer("Suggestion Approved", f"Suggestion `{suggestion_id}` approved successfully.")
        await interaction.followup.send(view=success.build(), ephemeral=True)
    ...
    async def deny_suggestion(self, interaction: discord.Interaction, suggestion_id: str, reason: str):
        ...
        success = KINETICHOSTSuccessContainer("Suggestion Denied", f"Suggestion `{suggestion_id}` denied successfully.")
        await interaction.followup.send(view=success.build(), ephemeral=True)
    ...
    async def implement_suggestion(self, interaction: discord.Interaction, suggestion_id: str, notes: str):
        ...
        if status != "approved":
            warn = KINETICHOSTWarningContainer("Status Conflict", f"Only approved suggestions can be marked as implemented (Current status: `{status}`).")
            await interaction.followup.send(view=warn.build(), ephemeral=True)
            return
        ...
        success = KINETICHOSTSuccessContainer("Suggestion Implemented", f"Suggestion `{suggestion_id}` marked as implemented.")
        await interaction.followup.send(view=success.build(), ephemeral=True)
    ...
    async def archive_suggestion(self, interaction: discord.Interaction, suggestion_id: str):
        ...
        if not row:
            err = KINETICHOSTErrorContainer("Suggestion Not Found", f"Suggestion `{suggestion_id}` does not exist.")
            await interaction.followup.send(view=err.build(), ephemeral=True)
            return
        ...
        success = KINETICHOSTSuccessContainer("Suggestion Archived", f"Suggestion thread for `{suggestion_id}` has been archived.")
        await interaction.followup.send(view=success.build(), ephemeral=True)
    ...
    async def suggestion_stats(self, interaction: discord.Interaction):
        ...
        stats_card = KINETICHOSTInfoContainer("Suggestions Analytics", f"**Server:** {interaction.guild.name}")
